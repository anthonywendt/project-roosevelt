...
while (( SECONDS < end )); do
  if k0s kubectl get nodes -o wide 2>/dev/null | grep -q "${NODE_IP}"; then
    break
  fi
  sleep 2
done

NODE_NAME="$(k0s kubectl get nodes -o wide 2>/dev/null | awk -v ip="${NODE_IP}" '$0 ~ ip {print $1; exit}')"
if [[ -z "${NODE_NAME}" ]]; then
...
k0s kubectl wait --for=condition=Ready "node/${NODE_NAME}" --timeout="${TIMEOUT_SECONDS}s"
